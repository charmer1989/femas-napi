...
let rspInfo = (params) => {
  if (params.some(s => s.match(/CUstpFtdcRspInfoField/))) {
    return `\n  if (pRspInfo) {
...
let CUstpFtdcParams = (params) => {
  if (params.some(s => s.match(/CUstpFtdc/) && !s.match(/CUstpFtdcRspInfoField/))) {
    let res = '';
    params.forEach(s => {
      if (s.match(/CUstpFtdc/) && !s.match(/CUstpFtdcRspInfoField/)) {
        let arr = s.match(/(CUstpFtdc[^\s]*)\s*\*([a-zA-Z]*)/);
        let paramType = arr[1];
        let paramName = arr[2];
        res = `\n  if (${paramName}) {
...
str.split("\n").forEach((line) => {
  ...
  TD_DEBUG_LOG("${funcName} called.");
  TdCbField *field                 = (TdCbField *)malloc(sizeof(TdCbField));
  uv_work_t *req                   = new uv_work_t;${rspInfo(params)}${nRequestID(params)}${CUstpFtdcParams(params)}
  TD_EVENT_CALL(field, ${_.replace(_.upperCase(_.snakeCase(funcName)), /\s/g, "_")}, 0);
};\n`;
